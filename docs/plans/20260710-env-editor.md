...
- [x] `docs/spec.md`: описать env-редактор, три корзины, правило классификации,
      shared-запись как точечный touched-only мердж, read-only нестроковых
      значений, запрет shared-ключей с managed-именами
- [x] `CLAUDE.md` (Инвариант 1): добавить строку, что пользователь может явно
...
- [x] сверить формулировки с реальным поведением из Task 1–4
